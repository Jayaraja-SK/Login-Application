...
connec=sqlite3.connect("password.db")
cursor=connec.cursor()
cursor.execute("create table if not exists info(email string primary key, salt string, password string);")
...
def add_user(email_id,pwd): # ADDING NEW USER'S INFORMATION TO THE DATABASE
    salt,pwd=generate_salt_pwd(pwd)

    sql='''insert into info(email,salt,password) values("'''+email_id+'''","'''+salt+'''","'''+pwd+'''")'''
...
def update_pwd_db(email_id,pwd):
    ...
    sql='''update info set salt="'''+salt+'''",password="'''+pwd+'''" where email="'''+email_id+'''"'''
...
def delete_info_db(email_id):
    sql='''delete from info where email="'''+email_id+'''"'''
